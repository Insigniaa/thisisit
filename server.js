...
// Static file serving middleware (moved to top)
...
async function getTwitchStreamerStatus(channelId) {
    return getCachedStatus('twitch', channelId, async () => {
        try {
            ...
            // First get user info to get profile picture
            ...
            let thumbnail = '/default-avatar.png';
            
            if (userData.data && userData.data.length > 0) {
                const profilePicUrl = userData.data[0].profile_image_url;
                if (profilePicUrl) {
                    const downloadResult = await downloadProfilePicture(profilePicUrl, channelId, 'twitch');
                    thumbnail = downloadResult.localPath;
                }
            }

            // Then get stream status
            const streamResponse = await fetch(`https://api.twitch.tv/helix/streams?user_login=${channelId}`, {
                headers: {
                    'Client-ID': process.env.TWITCH_CLIENT_ID,
                    'Authorization': `Bearer ${process.env.TWITCH_AUTH_TOKEN}`
                }
            ...
            return {
                isLive,
                viewers: isLive ? streamData.data[0].viewer_count : 0,
                title: isLive ? streamData.data[0].title : 'Offline',
                thumbnail: thumbnail
            };
        } catch (error) {
