...
async function checkStreamerBan(streamer) {
    const now = Date.now();
    const lastChecked = BAN_DETECTION_CONFIG.LAST_CHECKED.get(streamer.channelId);
    
    // Only check if we haven't checked recently
    if (lastChecked && (now - lastChecked < BAN_DETECTION_CONFIG.CHECK_INTERVAL)) {
        return { banned: streamer.banned || false };
    }
    
    BAN_DETECTION_CONFIG.LAST_CHECKED.set(streamer.channelId, now);
    
    // If manually banned, keep that status
    if (streamer.banned === true) {
        return { banned: true, reason: 'Manually banned' };
    }
    
    // Skip YouTube ban checks
    ...
    // Update streamer's ban status if banned
    if (banCheck.banned) {
        try {
            const data = await fs.readFile('streamers.json', 'utf8');
            const jsonData = JSON.parse(data);
            const streamerIndex = jsonData.streamers.findIndex(s => s.channelId === streamer.channelId);
            
            if (streamerIndex !== -1) {
                jsonData.streamers[streamerIndex].banned = true;
                jsonData.streamers[streamerIndex].banReason = banCheck.reason;
                await fs.writeFile('streamers.json', JSON.stringify(jsonData, null, 2));
            }
        } catch (error) {
            console.error('Error updating streamer ban status:', error);
        }
    }
...
const adminAuth = (req, res, next) => {
    const ADMIN_PASSWORD = process.env.ADMIN_PASSWORD; // In production, use environment variables
